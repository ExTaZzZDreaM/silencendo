...
describe("generateErrorDiagnostics", () => {
	...
	it("generates a diagnostics file with error metadata and history", async () => {
		...
		expect(result.filePath).toContain("s-code-diagnostics-")
		...
		expect(String(writtenPath)).toContain("s-code-diagnostics-test-tas")
		expect(String(writtenContent)).toContain(
			"// Please share this file with S-CODE Support (support@roocode.com) to diagnose the issue faster",
		)
