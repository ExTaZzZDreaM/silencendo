...
/**
 * Fetches available models from the S-CODE Cloud provider
 *
 * @param baseUrl The base URL of the S-CODE Cloud provider
 * @param apiKey The API key (session token) for the S-CODE Cloud provider
...
export async function getRooModels(baseUrl: string, apiKey?: string): Promise<ModelRecord> {
	...
	try {
		...
		try {
			...
			if (!parsed.success) {
				console.error("Error fetching S-CODE Cloud models: Unexpected response format", data)
				console.error("Validation errors:", parsed.error.format())
				throw new Error("Failed to fetch S-CODE Cloud models: Unexpected response format.")
			}
		...
		console.error("[getRooModels] Error fetching S-CODE Cloud models:", {
		...
		if (error.name === "AbortError") {
			throw new Error("Failed to fetch S-CODE Cloud models: Request timed out after 10 seconds.")
		}

		// Handle fetch errors
		if (error.message?.includes("HTTP")) {
			throw new Error(`Failed to fetch S-CODE Cloud models: ${error.message}. Check base URL and API key.`)
		}

		// Handle network errors
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch S-CODE Cloud models: No response from server. Check S-CODE Cloud server status and base URL.",
			)
		}

		throw new Error(`Failed to fetch S-CODE Cloud models: ${error.message || "An unknown error occurred."}`)
	}
}
