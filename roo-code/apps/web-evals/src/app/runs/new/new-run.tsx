...
export function NewRun() {
	...
	const onSubmit = useCallback(
		async (values: CreateRun) => {
			try {
				// Validate jobToken for S-CODE Cloud provider
				if (provider === "roo" && !values.jobToken?.trim()) {
					toast.error("S-CODE Cloud Token is required")
					return
				}
							...
							<FormItem>
								<Tabs
									...
									<TabsList className="mb-2">
										...
										<TabsTrigger value="roo">S-CODE Cloud</TabsTrigger>
							...
							render={({ field }) => (
								<FormItem>
									<div className="flex items-center gap-1">
										<FormLabel>S-CODE Cloud Token</FormLabel>
										<Tooltip>
											...
											<TooltipContent side="right" className="max-w-xs">
												<p>
													If you have access to the S-CODE Cloud repository and the
