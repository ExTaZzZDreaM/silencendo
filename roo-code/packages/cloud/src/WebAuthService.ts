...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(landingPageSlug?: string, useProviderSignup: boolean = false): Promise<void> {
		try {
			...
			this.log(`[auth] Error initiating S-CODE Cloud auth${context}: ${error}`)
			throw new Error(`Failed to initiate S-CODE Cloud authentication${context}: ${error}`)
		}
	}

	/**
	 * Handle the callback from S-CODE Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with S-CODE Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid S-CODE Cloud sign in url")
			}
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with S-CODE Cloud")
			}

			this.log("[auth] Successfully authenticated with S-CODE Cloud")
		} catch (error) {
			this.log(`[auth] Error handling S-CODE Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle S-CODE Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from S-CODE Cloud")
			}

			this.log("[auth] Logged out from S-CODE Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from S-CODE Cloud: ${error}`)
			throw new Error(`Failed to log out from S-CODE Cloud: ${error}`)
		}
	}
