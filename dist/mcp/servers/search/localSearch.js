...
function safePatternToRegex(pattern) {
    ...
    if (!p)
        return null;
    if (p.length > 128)
        throw new Error("file_pattern too long");
    const escaped = p
        .replace(/[.+^${}()|[\]\\]/g, "\\$&")
        .replace(/\*/g, ".*")
        .replace(/\?/g, ".");
...
function buildSnippet(content, idx) {
...
function scoreMatch(query, contentLower, firstIdx) {
...
async function isProbablyTextFile(absPath, maxBytes) {
...
async function walk(dirAbs, opts) {
    const results = [];
    const stack = [dirAbs];
    while (stack.length) {
...
export async function localSearch(query, options) {
    const q = query?.trim();
    if (!q)
        return [];
    if (q.length > 512)
    ...
    const startDirAbs = options.startDirRel
        ...
        : PROJECT_ROOT;
    if (!startDirAbs.startsWith(PROJECT_ROOT))
        throw new Error("Invalid start directory");
    const files = await walk(startDirAbs, { denyDirs, maxFiles });
    const matches = [];
    const qLower = q.toLowerCase();
    for (const absPath of files) {
        const ext = path.extname(absPath).toLowerCase();
        if (ext && !options.includeExtensions.has(ext))
            continue;
        ...
        if (options.dateTo && stat.mtime > options.dateTo)
            continue;
        if (!(await isProbablyTextFile(absPath, maxFileBytes)))
            continue;
