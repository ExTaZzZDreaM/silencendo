 ...
 */
export function validateCommandSecurity(command: string): void {
 ...
 */
export function detectImage(workingDir: string): string {
 ...
 */
export function getInterpreterImage(interpreter: string): string {
  const config = getConfig();
  const images = config.security.terminal.sandbox.docker_images;
 ...
 */
export function getScriptExtension(interpreter: string): string {
  const mapping: Record<string, string> = {
    'bash': 'sh',
    'sh': 'sh',
    'python': 'py',
...
export interface RunScriptParams {
  interpreter: 'bash' | 'python' | 'node' | 'go' | 'sh';
  script: string;
  args?: string[];
  working_dir?: string;
  timeout?: number;
...
export class DockerSandbox {
   ...
   */
  async runScript(params: RunScriptParams): Promise<CommandResult> {
    const startTime = Date.now();
